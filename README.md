...
> Строка, начинающаяся с `//`, является комментарием. Go поддерживает два вида комментариев: `//` превращает в комментарий весь текст до конца строки и `/* */`, где комментарием является всё, что содержится между симво-лами `*` (включая переносы строк).
...
> Строка — это последовательность символов определенной длины, ис-пользуемая для представления текста.  
...
> Переменные в Go создаются с помощью ключевого слова var, за кото-рым следуют имя переменной (x), тип (string) и присваиваемое значение (Hello World).  
Если мы хотим присвоить значение переменной при её создании, то мо-жем использовать сокращенную запись
...
Функция *panic* генерирует ошибку выполнения. Мы можем обрабаты-вать паники с помощью встроенной функции *recover*.
...
> Существует оператор `&`, который используется для получения адреса пе-ременной.

### 2.  Как присвоить значение указателю?

> Используя указатель (`*int`), мы можем изменить значение оригиналь-ной переменной.
...
> Разница в том, что при использовании метода не нужно использовать опе-ратор  '&' (Go автоматически предоставляет доступ к указателю на *Circle* для этого метода).
...
> Попытка получить сообщение из канала `c` вызовет ошибку компилиро-вания.  
...
> Буферизированный канал – это канал с определенной ёмкостью. Работа осуществляется асинхронно — получение или отправка сообщения не застав-ляют стороны останавливаться.
